...
export default class BattleScene extends Phaser.Scene {
    ...
    startFight(p1Id, p2Id, isVsAI) {
        // FIX: Restore visibility of scene objects that might have been hidden by UI.
        [...this.courtroomProps, ...this.galleryJudges].forEach(obj => obj.setVisible(true));
        ...
        this.timerEvent = this.time.addEvent({
            ...
            callback: () => {
                if (this.gameState === 'fight' && this.roundTimerValue > 0) {
                    ...
                    this.timerText.setText(this.roundTimerValue.toString());
                    if (this.roundTimerValue === 0) this.handleTimeOut();
                }
            },
            loop: true
        });
    ...
    handlePlayerInput(player, keys) {
        ...
        if (Phaser.Input.Keyboard.JustDown(keys.jump)) player.jump();
        if (Phaser.Input.Keyboard.JustDown(keys.attack)) player.triggerAttack();
        if (Phaser.Input.Keyboard.JustDown(keys.projectile)) player.triggerProjectile();
        if (Phaser.Input.Keyboard.JustDown(keys.groundPound)) player.triggerGroundPound();
        if (keys.charge.isDown) player.startCharging();
        if (Phaser.Input.Keyboard.JustUp(keys.charge)) player.releaseChargeAttack();
        if (Phaser.Input.Keyboard.JustDown(keys.super) && player.superMeter >= 100) this.triggerSuper(player);
    }
    ...
    createCourtroomProps() {
        ...
        this.courtroomProps.push(sunBeam1, sunBeam2, floorReflection);
    }
    ...
    createJudicialGallery() {
        ...
        for (let i = 0; i < 7; i++) {
            ...
            this.tweens.add({ targets: judgeContainer, y: jy + 6, duration: 900 + i * 150, yoyo: true, repeat: -1, ease: 'Sine.easeInOut' });
        }
    }
    ...
    transitionToCharacterSelect() {
        ...
        // FIX: Hide scene objects before showing UI to prevent them from being destroyed.
        [...this.courtroomProps, ...this.galleryJudges].forEach(obj => obj.setVisible(false));
        
        this.tweens.add({
            targets: [this.menuContainer, this.controlsContainer],
            alpha: 0,
            duration: 400,
            onComplete: () => {
    ...
    announceRoundStart() {
        ...
        const announcementText = this.add.text(width / 2, height / 2 - 20, 'ORAL ARGUMENT', { fontFamily: '"Press Start 2P"', fontSize: '52px', fill: '#fde047', stroke: '#000000', strokeThickness: 8 }).setOrigin(0.5).setDepth(16);
        const callText = this.add.text(width / 2, height / 2 + 40, 'Pleading Begins Now!', { fontFamily: '"Press Start 2P"', fontSize: '20px', fill: '#ffffff', stroke: '#000000', strokeThickness: 4 }).setOrigin(0.5).setDepth(16);
        ...
        this.tweens.timeline({
            tweens: [
                { targets: banner, scaleX: 1, duration: 350, ease: 'Quad.easeOut' },
                { targets: [announcementText, callText], scale: 1, duration: 250, ease: 'Back.easeOut', offset: '-=50' },
                { targets: [banner, announcementText, callText], alpha: 0, duration: 250, ease: 'Quad.easeIn', delay: 1000 }
            ],
            onComplete: () => [banner, announcementText, callText].forEach(el => el.destroy())
        });
    }
    ...
    updateHUD() {
        const width = this.scale.width;
        this.tweens.add({ targets: this.h1Bar, width: (this.player1.health / this.player1.maxHealth) * 594, duration: 20, ease: 'Linear' });
        this.tweens.add({ targets: this.h2Bar, x: width - 138 - (this.player2.health / this.player2.maxHealth) * 594, width: (this.player2.health / this.player2.maxHealth) * 594, duration: 20, ease: 'Linear' });
        ...
        this.s1Text.setText(this.player1.superMeter >= 100 ? 'SUPER READY! [SPACE]' : `METER  ${Math.floor(this.player1.superMeter)}%`).setFill(this.player1.superMeter >= 100 ? '#fde047' : '#a5b4fc');
        this.s2Bar.width = (this.player2.superMeter / 100) * 294;
        this.s2Bar.x = width - 142 - this.s2Bar.width;
        this.s2Text.setText(this.player2.superMeter >= 100 ? 'SUPER READY! [0]' : `METER  ${Math.floor(this.player2.superMeter)}%`).setFill(this.player2.superMeter >= 100 ? '#fde047' : '#fbcfe8');
    }
    ...
    handleAIBehavior() {
        ...
        if (dist > 250 && Math.random() > 0.95) this.player2.triggerProjectile();
        if (dist > 180) {
            this.player2.move(this.player1.x > this.player2.x ? 1 : -1);
            if (Math.random() < 0.02 && this.player2.isGrounded) this.player2.jump();
        } else {
            this.player2.body.setVelocityX(0);
            if (this.player2.superMeter >= 100) this.triggerSuper(this.player2);
            else if (this.time.now - this.player2.lastAttackTime > 350) this.player2.triggerAttack();
        }
    }

    triggerSuper(fighter) { /* Super move logic placeholder */ }
    ...
    cheerSpectators() {
        this.galleryJudges.forEach((judge, idx) => {
            ...
            if (judge.tinyGavel) this.tweens.add({ targets: judge.tinyGavel, angle: idx % 2 === 0 ? 35 : -35, duration: 80, yoyo: true, repeat: 3 });
        });
    }

    playSFX(key) { this.audioManager.playSFX(key); }

    showPopupText(x, y, text, color) {
        const t = this.add.text(x, y, text, { fontFamily: '"Press Start 2P"', fontSize: '18px', fill: color, stroke: '#000000', strokeThickness: 5, align: 'center' }).setOrigin(0.5).setDepth(25).setScale(0);
        this.tweens.timeline({
            tweens: [
                { targets: t, scale: 1.1, y: y - 55, duration: 180, ease: 'Back.easeOut' },
                { targets: t, alpha: 0, scale: 0.8, y: y - 90, duration: 250, delay: 550 }
            ],
    ...
    endFight(winner, loser) {
        ...
        this.fighters.forEach(f => f.body.setVelocity(0, 0).setAllowGravity(false));
        if (this.timerEvent) this.timerEvent.destroy();
        this.time.delayedCall(1200, () => {
            showFinalVerdictScreen(this, { id: winner.characterId, name: winner.charName }, { id: loser.characterId, name: loser.charName }, () => this.restartBattle());
        });
    }

    handleTimeOut() {
        const winner = this.player1.health >= this.player2.health ? this.player1 : this.player2;
        this.endFight(winner, winner === this.player1 ? this.player2 : this.player1);
    }

    checkVictoryCondition() {
        if (this.player1.health <= 0) this.endFight(this.player2, this.player1);
        else if (this.player2.health <= 0) this.endFight(this.player1, this.player2);
    }
