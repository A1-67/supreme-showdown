...
export default class BattleScene extends Phaser.Scene {
    ...
    createJudicialGallery() {
        ...
        // The background bench top sits at approximately y=595 in the courtroom image.
        // Judges are seated BEHIND it — we position them so only their upper body +
        // wig shows above the bench surface, giving a proper seated-behind-bench look.
        const benchTop   = 595;   // pixel y where bench surface sits
        const judgeCount = 7;
        const totalSpan  = 660;
        ...
        for (let i = 0; i < judgeCount; i++) {
            ...
            // Robe body — drawn downward from origin; hidden by bench mask below
            const robe = this.add.graphics();
            robe.fillStyle(0x111827, 1);
            robe.fillRoundedRect(-22, 0, 44, 55, 5);

            // White lace collar — sits right at the bench top line
            ...
            // Face — above origin (bench top)
            const face = this.add.graphics();
            face.fillStyle(0xfde047, 1);
            face.lineStyle(2, 0xd97706, 0.6);
            face.fillCircle(0, -18, 14);
            face.strokeCircle(0, -18, 14);

            // Expression — small stern eyes
            face.fillStyle(0x111827, 1);
            face.fillRect(-6, -21, 3, 3);
            face.fillRect(3,  -21, 3, 3);

            // Classic white curly judicial wig
            ...
            // Tiny gavel raised in right hand — resting on bench level
            const tinyGavel = this.add.graphics();
            tinyGavel.fillStyle(0x78350f, 1);
            tinyGavel.fillRect(14, -12, 4, 16);   // handle
            tinyGavel.fillStyle(0x451a03, 1);
            tinyGavel.fillRect(9, -20, 14, 7);    // head
            ...
            // Very subtle seated breathing bob — only ±4px so they look settled
            this.tweens.add({
    ...
    createMainMenu() {
        ...
        const p1Label = this.add.text(-220, 155, 'PLAYER 1: ROE', { fontFamily: '"Press Start 2P"', fontSize: '11px', fill: '#60a5fa' }).setOrigin(0.5);
        const p1Controls = this.add.text(-220, 240, 'Move: A/D  |  Jump: W\nKick: J  |  Slash: K\nTeleport: L  |  Tackle: U\nShield: S + J  |  Super: SPACE', { fontFamily: 'Arial', fontSize: '14px', fill: '#d1d5db', align: 'center', lineSpacing: 6 }).setOrigin(0.5);

        const p2Label = this.add.text(220, 155, 'PLAYER 2: WADE', { fontFamily: '"Press Start 2P"', fontSize: '11px', fill: '#fca5a5' }).setOrigin(0.5);
        const p2Controls = this.add.text(220, 240, 'Move: ←/→  |  Jump: ↑\nKick: NUM1  |  Slash: NUM2\nTeleport: NUM3  |  Tackle: NUM4\nShield: ↓ + NUM1  |  Super: NUM0', { fontFamily: 'Arial', fontSize: '14px', fill: '#d1d5db', align: 'center', lineSpacing: 6 }).setOrigin(0.5);
    ...
    setupControls() {
        ...
        // ─ ROE (P1) ────────────────────────────────────────────────────────
        // Move: A/D   Jump: W   Kick: J   Slash: K   Teleport: L   Tackle: U
        // Shield: S+J            Super: SPACE
        this.keysP1 = {
    ...
    update() {
        ...
        if (this.player2.isAI) {
            ...
            const p2kick     = Phaser.Input.Keyboard.JustDown(this.keysP2.kick)     || Phaser.Input.Keyboard.JustDown(this.keysP2.kickRow);
            const p2slash    = Phaser.Input.Keyboard.JustDown(this.keysP2.slash)    || Phaser.Input.Keyboard.JustDown(this.keysP2.slashRow);
            const p2teleport = Phaser.Input.Keyboard.JustDown(this.keysP2.teleport) || Phaser.Input.Keyboard.JustDown(this.keysP2.teleportRow);
            const p2tackle   = Phaser.Input.Keyboard.JustDown(this.keysP2.tackle)   || Phaser.Input.Keyboard.JustDown(this.keysP2.tackleRow);
            const p2super    = Phaser.Input.Keyboard.JustDown(this.keysP2.super)    || Phaser.Input.Keyboard.JustDown(this.keysP2.superRow);

            // Shield: hold ↓ then press kick key
            if (p2kick) {
                if (this.keysP2.down.isDown) this.player2.triggerSpecialShield();
                else                          this.player2.doKick();
            }
            if (p2slash)    this.player2.doSlash();
            if (p2teleport) this.player2.doTeleport();
            if (p2tackle)   this.player2.doTackle();

            if (p2super && this.player2.superMeter >= 100)
    ...
    handleAIBehavior() {
        ...
        if (dist > 220) {
            ...
            // Occasionally teleport towards player for surprise engagement
            if (dist > 500 && Math.random() < 0.008 && now - this.player2.lastAttackTime > 1200) {
                this.player2.doTeleport();
            }
    ...
    checkAttackOverlaps() {
        // Player 1 Hitbox hitting Player 2 (using geometric bounds to prevent any physical platforming side-effects!)
        if (this.player1.isAttacking) {
            const p1Box = this.player1.getAttackHitboxBounds();
            const p2Box = new Phaser.Geom.Rectangle(this.player2.body.x, this.player2.body.y, this.player2.body.width, this.player2.body.height);
            
            if (Phaser.Geom.Intersects.RectangleToRectangle(p1Box, p2Box)) {
        ...
        if (this.player2.isAttacking) {
            const p2Box = this.player2.getAttackHitboxBounds();
            const p1Box = new Phaser.Geom.Rectangle(this.player1.body.x, this.player1.body.y, this.player1.body.width, this.player1.body.height);
            
            if (Phaser.Geom.Intersects.RectangleToRectangle(p2Box, p1Box)) {
    ...
    cheerSpectators() {
        // Judges pop up from behind the bench and wave gavels on big hits
